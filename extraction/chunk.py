"""The Chunk schema — the extraction stage's output contract.

A Chunk is the unit the rest of the RAG pipeline embeds, stores in Chroma, retrieves,
and cites. It carries everything two downstream roles need:
...
synthesis model context, `highlight_span` gives it precision. Chunks are built for this
dual use from the start, never retrofitted.

Provenance fields are copied straight off the parent Document — they must survive into
Chroma so synthesis can always cite source + timestamp. Construction lives in `build_chunk`
(and `materialize_chunks`), keeping the copy in one place so no chunker repeats it, exactly
as Document construction lives in the normalizer rather than on the dataclass.
"""
...
class Chunk:
    # --- identity ---
    chunk_id: str  # deterministic: parent document_id + "#" + ordinal
...
def build_chunk(
    ...
    `ordinal` is the chunk's 0-based position within the document; combined with the
    parent id it yields the deterministic `chunk_id` (e.g. "8f3c2a…e1#0"). `chunked_at`
    is injectable so callers (and tests) can pin the stamp; it defaults to now (UTC).
    """
    return Chunk(
...
def materialize_chunks(
    ...
    *,
    ...
    strategy planned over*. `base` shifts them into absolute parent-body coordinates (0 for
    a whole-body pass; the section start when a chunker splits a sub-section), and
    `start_ordinal` continues chunk numbering across sub-passes. The chunk text is sliced
    from the parent body by the absolute span, so it always matches `full_span` exactly.
    """
    chunks: list[Chunk] = []
