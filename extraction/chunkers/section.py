...
SEC filings and regulatory text are organized under explicit headers ("RISK FACTORS",
"Item 1A."), and their sections are the meaningful retrieval unit (CLAUDE.md: "filings by
section"). This splits the body at detected headers (via utils/section_detection.py) and
emits one chunk per section — except a section too large to embed well, which is recursively
broken with a fallback strategy (paragraph by default, fixed on request).

`highlight_span` is the first sentence *after* the header line (the Phase 1 heuristic).
Section labels are noted here but are not yet a formal Chunk field — a documented Phase 1
non-goal.

Note on current data: EDGAR discovery is metadata-only (`expects.body = false`), so real
EDGAR Documents have empty bodies and produce no section chunks. This strategy is exercised
against filing bodies once a full-text filing source is onboarded; tests use a synthetic
filing body meanwhile.
"""

from typing import TYPE_CHECKING

from extraction.chunk import Chunk, Span, materialize_chunks
from extraction.chunkers.fixed import _plan_fixed
from extraction.chunkers.paragraph import _plan_paragraph
...
# A section is (start, end, content_start): body bounds plus where content begins after
# the header line (None for a header-less preamble, where content starts at `start`).
...
def chunk_section(
    ...
    body = document.body
    if not body:
        return []  # no content to section (e.g. EDGAR's metadata-only Documents)
    chunks: list[Chunk] = []
    ordinal = 0
    for start, end, content_start in _split_sections(body):
        segment = body[start:end]
        if count_tokens(segment) > max_section_tokens:
