...
place that model is named, mirroring `extraction.utils.tokenization`'s lazy-load pattern for
...
def embed_text(text: str) -> list[float]:
    ...
    `.tolist()`, not `list(...)`: `model.encode()` returns a numpy array, and `list()` on
    a numpy array yields numpy float32 scalars, not native Python floats. Chroma's
    `query_embeddings` validation rejects those — `.tolist()` recursively converts to
    plain Python floats, the same fix `extraction/embedder.py`'s batch encode() applies.
    """
    model = _get_model()
