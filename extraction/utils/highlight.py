...
Non-goal for now — model-ranked "key sentence" selection (Phase 2+); the signature stays
...
def select_highlight_span(text: str, start: int = 0) -> tuple[int, int]:
    ...
    `start` lets a caller skip a leading header line (the section chunker passes the
    offset just past the header). Falls back to the whole trimmed remainder when the
    text has no sentence terminator.
    """
