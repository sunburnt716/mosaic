"""Contract tests for the fixed-size chunker (extraction/chunkers/fixed.py).

Uses the word-level FakeTokenizer, so "tokens" here are whitespace words: a body of N words
with `chunk_size`/`overlap` in words lets us pin window boundaries, overlap, ordinals, and
the fixed-chunk rule that highlight_span == full_span.
"""
...
class TestChunkFixed:
    def test_windows_with_overlap(self, fake_tokenizer):
        doc = make_document(body=_BODY)
        chunks = chunk_fixed(doc, chunk_size=4, overlap=1, chunked_at="t")
    ...
    def test_ordinals_are_contiguous(self, fake_tokenizer):
        doc = make_document(id="docid", body=_BODY)
        chunks = chunk_fixed(doc, chunk_size=4, overlap=1, chunked_at="t")
        assert [c.chunk_id for c in chunks] == ["docid#0", "docid#1", "docid#2"]

    def test_body_smaller_than_window_is_one_chunk(self, fake_tokenizer):
        doc = make_document(body="alpha beta gamma")
        chunks = chunk_fixed(doc, chunk_size=512, overlap=50, chunked_at="t")
        assert len(chunks) == 1
