"""Contract tests for the paragraph chunker (extraction/chunkers/paragraph.py).

Paragraphs are newline-separated (as the normalizer emits them). With the word-level
FakeTokenizer, `min_paragraph_tokens` is a word count, letting us pin the orphan-merge
...
class TestChunkParagraph:
    ...
    def test_highlight_is_first_sentence(self, fake_tokenizer):
        ...
        chunks = chunk_paragraph(doc, min_paragraph_tokens=1)
        first = chunks[0]
