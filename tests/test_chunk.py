"""Contract tests for the Chunk dataclass and its builders (extraction/chunk.py).
...
from extraction.chunk import build_chunk, materialize_chunks
...
class TestBuildChunk:
    ...
    def test_provenance_copied_from_document(self):
        ...
        assert chunk.tier == 1
        assert chunk.published_date == doc.published_date
    ...
    def test_chunked_at_defaults_to_iso_utc(self):
        ...
        # Parseable ISO 8601 and timezone-aware.
...
class TestMaterializeChunks:
    ...
    def test_base_offset_shifts_spans_and_ordinals(self):
        doc = make_document(body="0123456789")
        plans = [((0, 3), (0, 1))]
        chunks = materialize_chunks(doc, plans, base=4, start_ordinal=2, chunked_at="t")
