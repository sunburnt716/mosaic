"""Contract tests for extraction/utils/highlight.py.

The highlight span is the excerpt generation cites back to the user. Phase 1 heuristic:
the first sentence, optionally past a leading header offset. These pin that behaviour and
the whole-remainder fallback when no sentence terminator exists.
"""
...
class TestSelectHighlightSpan:
    ...
    def test_start_offset_skips_header(self):
        ...
        start, end = select_highlight_span(text, start=header_end)
